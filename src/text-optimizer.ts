export class TextOptimizer {
    ...
    async optimizeText(text: string, onProgress?: (text: string) => void): Promise<string> {
        ...
        const systemPrompt = isChineseText ? 
            // 中文优化提示词
            `作为专业的视频字幕编辑专家，请优化以下字幕文本，使其更加流畅自然。要求：
1. 保持原文的语义和语气
2. 修正不自然的表达
3. 保持语义连贯性和完整性
4. 保留所有重要信息
...
6. 保持专业术语不变
7. 确保优化后的文本听起来自然
8. 直接输出优化后的文本，不要添加任何说明或前缀
9. 不要改变原文的分段和格式` :
            // 英文优化提示词
            `As a professional video subtitle editor, please optimize the following text. Requirements:
...
8. Output the optimized text directly, without any prefix or explanation
